...
sudo docker run --rm --name angular-cli -v ${PWD}:/frontend -w /frontend node:8.15.1 /bin/bash -c "npm install -g @angular/cli; npm install; npm rebuild; ng build --prod --baseHref=/new/"
cd dist
sudo mv my-app $HOME/santatecla-relaciones-2/relman/src/main/resources/static
